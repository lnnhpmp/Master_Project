...
bool CParticleSystemTask::InitResources(cl_device_id Device, cl_context Context)
{
	...
	V_RETURN_FALSE_CL(clError, "Failed to set args 1 for m_IntegrateKernel");
	...
	V_RETURN_FALSE_CL(clError, "Failed to set args 2 for m_IntegrateKernel");
    clError |= clSetKernelArg(m_IntegrateKernel, 3, sizeof(cl_uint), (void*)&m_nParticles);
	
	V_RETURN_FALSE_CL(clError, "Failed to set args 3 for m_IntegrateKernel");
    clError |= clSetKernelArg(m_IntegrateKernel, 4, sizeof(cl_uint), (void*)&m_nTriangles);
	V_RETURN_FALSE_CL(clError, "Failed to set args 4 for m_IntegrateKernel");
...
void CParticleSystemTask::Scan(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	unsigned numPartTwice = m_nParticles * 2;
	clEnqueueCopyBuffer(CommandQueue, m_clAlive, m_clPingArray, 0, 0, numPartTwice*sizeof(cl_uint), 0, NULL, NULL);
	clEnqueueBarrier(CommandQueue);
	int reverse = 0;
	for (offset = 1; offset < numPartTwice; offset *= 2)
	{
		globalWorkSize[0] = numPartTwice;
		...
		if (reverse == 0)
		{
			clErr = clSetKernelArg(m_ScanNaiveKernel, 0, sizeof(cl_mem), (void*)&m_clPingArray);
			clErr |= clSetKernelArg(m_ScanNaiveKernel, 1, sizeof(cl_mem), (void*)&m_clPongArray);
		}
		else
		{
			clErr = clSetKernelArg(m_ScanNaiveKernel, 0, sizeof(cl_mem), (void*)&m_clPongArray);
			clErr |= clSetKernelArg(m_ScanNaiveKernel, 1, sizeof(cl_mem), (void*)&m_clPingArray);
		}
		clErr |= clSetKernelArg(m_ScanNaiveKernel, 2, sizeof(cl_uint), (void*)&numPartTwice);
		clErr |= clSetKernelArg(m_ScanNaiveKernel, 3, sizeof(cl_uint), (void*)&offset);
		V_RETURN_CL(clErr, "Failed to set kernel args: Scan_Naive");

		//cout << "gl:" << globalWorkSize[0] << "ll:" << localWorkSize[0] << endl;
		clErr = clEnqueueNDRangeKernel(CommandQueue, m_ScanNaiveKernel, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
		V_RETURN_CL(clErr, "Error executing m_ScanNaiveKernel!");
	...
	if (reverse == 1) //reverse was previously 0, meaning previous result was written into "pong"
	{
...
void CParticleSystemTask::Integrate(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3], float dT)
{
	...
	V_RETURN_CL(clErr, "Failed to set args 5 for m_IntegrateKernel");
    clErr |= clSetKernelArg(m_IntegrateKernel, 6, sizeof(cl_mem), (void*)&m_clTriangleSoup);
	
	V_RETURN_CL(clErr, "Failed to set args 6 for m_IntegrateKernel");
    clErr |= clSetKernelArg(m_IntegrateKernel, 7, sizeof(cl_mem), (void*)&m_clPosLife[0]);
	
	V_RETURN_CL(clErr, "Failed to set args 7 for m_IntegrateKernel");
    clErr |= clSetKernelArg(m_IntegrateKernel, 8, sizeof(cl_mem), (void*)&m_clVelMass[0]);
	
	V_RETURN_CL(clErr, "Failed to set args 8 for m_IntegrateKernel");
    clErr |= clSetKernelArg(m_IntegrateKernel, 9, sizeof(cl_float), (void*)&dT);
	V_RETURN_CL(clErr, "Failed to set args 9 for m_IntegrateKernel");
...
void CParticleSystemTask::Reorganize(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	cl_int clErr;
	size_t globalWorkSize[1];
	globalWorkSize[0] = CLUtil::GetGlobalWorkSize(2*m_nParticles, LocalWorkSize[0]);
	...
	clErr |= clSetKernelArg(m_ReorganizeKernel, 1, sizeof(cl_mem), (void*)&m_clPingArray); //m_clLevelArrays[0] array in case of efficient scan
